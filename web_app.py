...
def layout(content, page="dashboard"):
    user = current_user()
    items = [("dashboard","Dashboard",ICON_DASH,"/"),("new","New Pipeline",ICON_PLUS,"/new"),("minigame","New Mini-Game","🎮","/new-minigame"),("novel","Novel Game","🧪","/novel"),("arcade","Arcade",ICON_ARCADE,"/arcade"),("library","Game Library","📚","/game-library"),("operator","Operator","📊","/operator"),("recon","State Recon",ICON_GLOBE,"/recon"),("reviews","Reviews",ICON_REVIEW,"/reviews"),("history","History",ICON_CLOCK,"/history"),("files","All Files",ICON_FOLDER,"/files"),("qdrant","Qdrant",ICON_DB,"/qdrant"),("settings","Settings",ICON_SETTINGS,"/settings"),("admin","⚡ Control Plane","⚡","/admin/")]
...
def arcade_hub():
    ...
    <div class="arcade-links">
        ...
        <a href="/operator" style="background:linear-gradient(135deg,#22c55e,#16a34a);color:#fff;font-weight:700;border-color:transparent;padding:8px 18px">📊 Operator</a>
...
def api_novel_generate():
    ...
    try:
        ...
        safe_name = re.sub(r'[^a-z0-9]+', '-', mechanic.name.lower()).strip('-')
        filename = f"novel_{safe_name}_{mechanic.mechanic_hash[:8]}.html"
        (gen_dir / filename).write_text(html, encoding="utf-8")

        # Playtest
        simulator = PlaytestSimulator()
        report = simulator.evaluate(mechanic, n_rounds=50_000)

        return jsonify({
            "mechanic": mechanic.to_dict(),
            "playtest": report.to_dict(),
...
def operator_dashboard():
    """Operator dashboard — revenue, RTP monitoring, player stats."""
    try:
        pe = _get_platform()
        dash = pe.get_realtime_dashboard()
        stats = pe.get_game_stats(days=30)
        jackpots = pe.get_jackpots()
    except Exception:
        dash = {"active_sessions": 0, "today": {"rounds": 0, "wagered": 0, "won": 0, "ggr": 0, "rtp": 0}, "jackpots": [], "top_games": []}
        stats = {"games": [], "totals": {"wagered": 0, "won": 0, "ggr": 0, "overall_rtp": 0}}
        jackpots = []

    t = dash["today"]
    game_rows = ""
    for g in stats["games"]:
        rtp_color = "#22c55e" if 94 < g["rtp_measured"] < 98 else "#f59e0b"
        game_rows += (
            f'<div class="op-row">'
            f'<span class="op-game">{g["game_type"].title()}</span>'
            f'<span>{g["sessions"]}</span>'
            f'<span>{g["rounds"]:,}</span>'
            f'<span>${g["wagered"]:,.2f}</span>'
            f'<span>${g["ggr"]:,.2f}</span>'
            f'<span style="color:{rtp_color};font-weight:600">{g["rtp_measured"]:.1f}%</span>'
            f'<span>{g["max_multiplier"]:.0f}x</span>'
            f'</div>'
        )
    if not game_rows:
        game_rows = '<div class="empty-state" style="padding:20px"><p>No game data yet. Play some rounds to see stats.</p></div>'

    jp_cards = ""
    for jp in jackpots:
        jp_cards += f'<div class="jp-card"><div class="jp-name">{jp["name"]}</div><div class="jp-amount">${jp["current_amount"]:,.2f}</div></div>'

    return layout(f'''
    <style>
    .op-grid{{display:grid;grid-template-columns:repeat(4,1fr);gap:16px;margin-bottom:24px}}
    @media(max-width:768px){{.op-grid{{grid-template-columns:1fr 1fr}}}}
    .op-stat{{background:var(--bg-card);border:1px solid var(--border);border-radius:12px;padding:16px;text-align:center}}
    .op-stat .label{{font-size:11px;color:var(--text-dim);text-transform:uppercase;letter-spacing:.5px}}
    .op-stat .value{{font-size:24px;font-weight:700;margin-top:4px}}
    .op-stat .sub{{font-size:11px;color:var(--text-dim);margin-top:2px}}
    .op-table{{display:grid;grid-template-columns:1fr repeat(6,.8fr);gap:0;font-size:12px}}
    .op-header{{display:contents}}
    .op-header span{{padding:8px 12px;font-weight:600;color:var(--text-dim);text-transform:uppercase;font-size:10px;letter-spacing:.5px;border-bottom:1px solid var(--border)}}
    .op-row{{display:contents}}
    .op-row span{{padding:10px 12px;border-bottom:1px solid rgba(255,255,255,0.03)}}
    .op-game{{font-weight:600;color:var(--text-bright)}}
    .jp-grid{{display:flex;gap:12px;flex-wrap:wrap;margin-top:12px}}
    .jp-card{{background:linear-gradient(135deg,#7c3aed,#ec4899);border-radius:10px;padding:14px 20px;min-width:140px}}
    .jp-name{{font-size:11px;opacity:.8}}
    .jp-amount{{font-size:20px;font-weight:700;margin-top:2px}}
    </style>
    <h2 class="page-title">📊 Operator Dashboard</h2>
    <p class="page-subtitle">Real-time revenue, RTP monitoring, and player analytics.</p>

    <div class="op-grid">
        <div class="op-stat"><div class="label">Active Sessions</div><div class="value" style="color:#22c55e">{t.get("rounds",0) > 0 and "🟢" or "⚪"} {dash["active_sessions"]}</div></div>
        <div class="op-stat"><div class="label">Today's Wagers</div><div class="value">${t["wagered"]:,.2f}</div><div class="sub">{t["rounds"]:,} rounds</div></div>
        <div class="op-stat"><div class="label">GGR (Revenue)</div><div class="value" style="color:{"#22c55e" if t["ggr"] >= 0 else "#ef4444"}">${t["ggr"]:,.2f}</div></div>
        <div class="op-stat"><div class="label">Measured RTP</div><div class="value">{t["rtp"]:.1f}%</div></div>
    </div>

    <div class="card">
        <h2>Game Performance (30 days)</h2>
        <div class="op-table">
            <div class="op-header"><span>Game</span><span>Sessions</span><span>Rounds</span><span>Wagered</span><span>GGR</span><span>RTP</span><span>Max Win</span></div>
            {game_rows}
        </div>
        <div style="margin-top:12px;font-size:12px;color:var(--text-dim)">
            Totals: wagered=${stats["totals"]["wagered"]:,.2f} | GGR=${stats["totals"]["ggr"]:,.2f} | RTP={stats["totals"]["overall_rtp"]:.1f}%
        </div>
    </div>

    {"<div class='card'><h2>💎 Progressive Jackpots</h2><div class='jp-grid'>" + jp_cards + "</div></div>" if jp_cards else ""}
    ''', "arcade")
...
@app.route("/api/platform/session", methods=["POST"])
@login_required
def api_platform_session():
    """Create a server-side game session."""
    try:
        pe = _get_platform()
        data = request.get_json(silent=True) or {}
        user = current_user()
        session = pe.create_session(
            user_id=user["id"],
            game_type=data.get("game_type", "crash"),
...
@app.route("/api/platform/play", methods=["POST"])
@login_required
def api_platform_play():
    """Play a round in a server-side session."""
    try:
        pe = _get_platform()
        data = request.get_json(silent=True) or {}
        result = pe.play_round(
...
@app.route("/api/platform/close", methods=["POST"])
@login_required
def api_platform_close():
    """Close a session and reveal server seed."""
    try:
        pe = _get_platform()
        data = request.get_json(silent=True) or {}
        result = pe.close_session(data["session_id"])
...
@login_required
def api_platform_verify():
    """Verify a round after session close."""
    try:
        pe = _get_platform()
        data = request.get_json(silent=True) or {}
        result = pe.verify_round(data["session_id"], int(data["nonce"]))
        return jsonify(result)
    except Exception as e:
        return jsonify({"error": str(e)}), 400
...
def api_platform_stats():
    """Player stats for current user."""
    try:
        pe = _get_platform()
        user = current_user()
        return jsonify(pe.get_player_stats(user["id"]))
    except Exception as e:
        return jsonify({"error": str(e)}), 400


@app.route("/api/platform/operator")
@login_required
def api_platform_operator():
    ...
    try:
        ...
        return jsonify({
            "dashboard": pe.get_realtime_dashboard(),
            "game_stats": pe.get_game_stats(game_type=game_type, days=days),
            "jackpots": pe.get_jackpots(),
        })
    except Exception as e:
        return jsonify({"error": str(e)}), 400
...
@app.route("/api/platform/library")
@login_required
def api_platform_library():
    """Search game library."""
    try:
        pe = _get_platform()
        return jsonify(pe.search_games(
            query=request.args.get("q", ""),
            game_type=request.args.get("type", ""),
            volatility=request.args.get("vol", ""),
